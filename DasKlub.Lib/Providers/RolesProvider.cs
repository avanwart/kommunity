...
namespace DasKlub.Lib.Providers
{
    public class DKRoleProvider : RoleProvider
    {
        ...
        public override bool IsUserInRole(string username, string roleName)
        {
            ...
            foreach (string s in userRoles.Where(s => roleName == s))
            {
                isUserInRole = true;
            }
            return isUserInRole;
        ...
        {
            foreach (UserAccount eu in usernames.Select(t => new UserAccount(t)).Where(eu => eu.UserAccountID > 0))
            {
                foreach (string t in roleNames)
                {
